...
## Steps
1. Download the latest version from the [Releases](https://github.com/SubConv/SubConv/releases) page and extract it to your machine, for example `/opt/subconv`. Keep the extracted `api` executable and the `mainpage/dist` directory together so the Web UI can be served.
...
5. Run `./api -h` to see the help message.
   For example, you can use `./api -H 127.0.0.1 -P 2534` to specify the host and port to listen on. If all parameters are omitted, the host will be `0.0.0.0` and the port will be `8080`.
